...
sys_mmap(){
  ...
  if(argaddr(0, &addr) < 0 || argaddr(1, &sz) < 0 || argint(2, &prot) < 0
    || argint(3, &flags) < 0 || argfd(4, &fd, &pf) < 0 || argaddr(5, &offset) < 0 || sz == 0)
    return -1;

  if((!pf->readable && (prot & (PROT_READ)))
     || (!pf->writable && (prot & PROT_WRITE) && !(flags & MAP_PRIVATE)))
    return -1;
  ...
  if(v == 0)
    panic("mmap: failed to allocat vma");
...
int vmatrylazytouch(uint64 va) {
  ...
  begin_op();
  ilock(v->pf->ip);
  readi(v->pf->ip, 0, (uint64)pa, v->offset + PGROUNDDOWN(va - v->vastart), PGSIZE);
  iunlock(v->pf->ip);
  end_op();
